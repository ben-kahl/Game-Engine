...
{	
	...
	if (!p1Texture.loadFromFile("Textures/dot.bmp"))
	{
		return success;
	}
	SpriteSheet p1SpriteSheet(p1Texture, 1, 1);
	...
	Button button(SCREEN_WIDTH /2, SCREEN_HEIGHT /2, &spriteSheets[1]);
	buttons.push_back(button);
	Character p1(levels[3].getLevW() / 2, SCREEN_HEIGHT / 1.4, &spriteSheets[0]);
	characters.push_back(p1);

	Character p2(levels[3].getLevW() / 2, SCREEN_HEIGHT / 2, &spriteSheets[0]);
